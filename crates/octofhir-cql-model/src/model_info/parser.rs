...
use super::types::{ModelInfo, PropertyInfo, TypeInfo};
    ...
    loop {
        match reader.read_event_into(&mut buf) {
                    ...
                    "modelInfo" => {
                        ...
                        for attr in e.attributes() {
                            ...
                            match key.as_ref() {
                                ...
                                "targetQualifier" => {
                                    model_info.target_qualifier = Some(value.to_string())
                                }
                                "patientClassName" => {
                                    model_info.patient_class_name = Some(value.to_string())
                                }
                    ...
                    "typeInfo" | "classInfo" | "simpleTypeInfo" | "profileInfo" => {
                        ...
                        for attr in e.attributes() {
                            ...
                            match key.as_ref() {
                                ...
                                "retrievable" => type_info.retrievable = value.as_ref() == "true",
                                "primaryCodePath" => {
                        ...
                        if current_type.is_some() {
                            ...
                            for attr in e.attributes() {
                                ...
                                match key.as_ref() {
                                    ...
                                    "type" | "elementType" => {
                                        property.element_type = value.to_string()
                                    }
                            ...
                            if property.element_type.starts_with("list<")
                                || property.element_type.starts_with("List<")
                            {
                                ...
                                if let Some(inner) = property
                                    .element_type
                                    .strip_prefix("list<")
                                    .or_else(|| property.element_type.strip_prefix("List<"))
            ...
            Ok(Event::Empty(e)) => {
                ...
                if tag_name.as_str() == "element" && current_type.is_some() {
                    let mut property = PropertyInfo::new("", "");
                ...
                match tag_name.as_str() {
                    ...
                    "element" => {
                        if let Some(property) = current_property.take()
                            && let Some(ref mut type_info) = current_type
                        {
                            type_info.elements.push(property);
                        }
                    }
...
pub fn parse_json(json_content: &str) -> Result<ModelInfo, ParseError> {
    ...
    model_info.patient_birth_date_property_name = json["patientBirthDatePropertyName"]
        .as_str()
        .map(String::from);
...
fn parse_type_info_json(json: &JsonValue) -> Result<TypeInfo, ParseError> {
    let mut type_info = TypeInfo::new(
        json["name"]
            .as_str()
            .ok_or_else(|| ParseError::InvalidStructure("Missing type name".to_string()))?,
    );
...
fn parse_property_json(json: &JsonValue) -> Result<PropertyInfo, ParseError> {
    let name = json["name"]
        .as_str()
        .ok_or_else(|| ParseError::InvalidStructure("Missing element name".to_string()))?;
    ...
    if element_type.starts_with("list<") || element_type.starts_with("List<") {
        is_list = true;
        // Extract inner type from list<T>
        if let Some(inner) = element_type
            .strip_prefix("list<")
            .or_else(|| element_type.strip_prefix("List<"))
            && let Some(inner) = inner.strip_suffix('>')
        {
...
mod tests {
    ...
    fn test_parse_simple_xml() {
        ...
        assert!(patient_type.retrievable);
